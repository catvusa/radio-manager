...
{	
	// Get test data
	let radio_data = getRadioStationData();
	
	// Set a new radio station
	let radio_station = new RMRadioStation(
		radio_data["name"],
		radio_data["logo"],
		radio_data["imgDuration"],
		radio_data["postType"],
		radio_data["player"],
		radio_data["playlist"],
		radio_data["warnings"]
	...
	radio_station.setValuesToHTML();
	radio_station.play();	
})

function getRadioStationData()
{	
	let classical_jingles =
	[
		{
			name: "CATV",
			description: "",
	...
	return {
		...
		"imgDuration": 20,
		"postType": "",
		"player": "rm-radio-station-player",
		"playlist": [
			{
